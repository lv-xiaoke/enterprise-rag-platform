...
[预期创建的 Docker Compose 配置](../../docker-compose.yml)
...
在 `docker-compose.yml` 中只定义一个 `postgres` 服务和一个命名卷 `postgres_data`。配置应包含以下骨架；你要自己补全缩进并理解每一项的作用：
...
创建 `.env.example`，只放公开的本地开发键名和非真实示例值，例如：
...
然后由你在本机复制 `.env.example` 为 `.env`，将密码改成只属于本机的值。`.gitignore` 已忽略 `.env`，但 `.env.example` 必须保留为可提交的配置说明。

健康检查使用 `$$POSTGRES_USER` 而不是 `$POSTGRES_USER`，是为了让 Compose 把变量原样交给容器内的 shell；否则 Compose 会在宿主机先展开它。端口绑定到 `127.0.0.1`，使本日数据库只供本机开发访问，不意外暴露给局域网。

今天没有建议直接外包给 AI 的核心工作：Compose 的字段、环境变量边界与健康检查是你需要亲手理解和配置的内容。
